...
void CandPlot::plot(const Cluster &cluster, const Boxcar &boxcar, const RealTime::SubbandDedispersion &dedisp, double tstart, float threS, const string &rootname, int id, int fileid, std::string &fname, std::map<std::string, std::string> &obsinfo)
{
    ...
    for (long int iidx=0; iidx<ncand; iidx++)
    {
        ...
        double dmpos = boxcar.dms+idm*boxcar.ddm;
        ...
        ax_dmS.annotate("DM = "+s_dm+" cm\\u-3\\dpc", snr*0.75, *std::min_element(vdm_down.begin(), vdm_down.end()), {{"xycoords", "data"}, {"rotation", "270"}, {"refpos", "right"}});
        ...
        ax_meta.annotate(fname, 0.985, 0.1, {{"xycoords","figure fraction"}, {"fontsize", "0.7"}, {"rotation", "270"}});
