...
public:
	...
	~CandPlot();
	void plot(const Cluster<double> &cluster, const Boxcar &boxcar, const RealTime::SubbandDedispersion &dedisp, int id, int fileid, std::string &fname, std::map<std::string, std::string> &obsinfo, bool saveimage=false, bool white=false);
private:
