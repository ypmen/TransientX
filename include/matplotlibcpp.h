...
struct _interpreter {
    ...
    PyObject *s_python_function_autoscale;
    PyObject *s_python_function_ticklabel_format;
    PyObject *s_python_function_pcolormesh;
    PyObject *s_python_function_twinx;
...
#  endif
    ...
    _interpreter() {
        ...
        s_python_function_autoscale = safe_import(pymod, "autoscale");
        s_python_function_ticklabel_format = safe_import(pymod, "ticklabel_format");
        s_python_function_pcolormesh = safe_import(pymod, "pcolormesh");
        s_python_function_twinx = safe_import(pymod, "twinx");
#ifndef WITHOUT_NUMPY
...
inline bool annotate(std::string annotation, double x, double y, const std::map<std::string, std::string> &keywords)
{
    ...
    if (!res)
        throw std::runtime_error("Call to annotation() failed");
    Py_DECREF(res);
...
inline void pcolormesh(void *x, void *y, void *Z, const NPY_TYPES type, const int rows, const int columns, const std::map<std::string, std::string> &keywords)
{
    assert(type == NPY_FLOAT || type == NPY_DOUBLE);

    detail::_interpreter::get();

    // construct args
    npy_intp zdims[2] = {rows, columns};
    npy_intp xdims[1] = {columns};
    npy_intp ydims[1] = {rows};
    PyObject *args = NULL;
    if (x != NULL and y != NULL)
    {
        args = PyTuple_New(3);
        PyTuple_SetItem(args, 0, PyArray_SimpleNewFromData(1, xdims, type, x));
        PyTuple_SetItem(args, 1, PyArray_SimpleNewFromData(1, ydims, type, y));
        PyTuple_SetItem(args, 2, PyArray_SimpleNewFromData(2, zdims, type, Z));
    }
    else
    {
        args = PyTuple_New(1);
        PyTuple_SetItem(args, 0, PyArray_SimpleNewFromData(2, zdims, type, Z));
    }
    ...
    PyDict_SetItemString(kwargs, "shading", PyString_FromString("auto"));

    PyObject *res = PyObject_Call(detail::_interpreter::get().s_python_function_pcolormesh, args, kwargs);
...
inline void twinx()
{
    ...
    PyObject *res = PyObject_CallObject(detail::_interpreter::get().s_python_function_twinx, args);
...
inline void pcolormesh(std::vector<float> &x, std::vector<float> &y, std::vector<float> &Z, const std::map<std::string, std::string> &keywords = {})
{
    int rows = y.size();
    int columns = x.size();

    detail::pcolormesh((void *)&x[0], (void *)&y[0], (void *)&Z[0], NPY_FLOAT, rows, columns, keywords);
}

inline void pcolormesh(int ncols, int nrows, std::vector<float> &Z, const std::map<std::string, std::string> &keywords = {})
{
    int rows = nrows;
    int columns = ncols;

    detail::pcolormesh(NULL, NULL, (void *)&Z[0], NPY_FLOAT, rows, columns, keywords);
}


inline void pcolormesh(std::vector<double> &x, std::vector<double> &y, std::vector<double> &Z, const std::map<std::string, std::string> &keywords = {})
{
    int rows = y.size();
    int columns = x.size();

    detail::pcolormesh((void *)&x[0], (void *)&y[0], (void *)&Z[0], NPY_DOUBLE, rows, columns, keywords);
}

inline void pcolormesh(int ncols, int nrows, std::vector<double> &Z, const std::map<std::string, std::string> &keywords = {})
{
    int rows = nrows;
    int columns = ncols;

    detail::pcolormesh(NULL, NULL, (void *)&Z[0], NPY_DOUBLE, rows, columns, keywords);
}
...
inline void autoscale(bool enable=true, std::string axis="both", bool tight=true)
{
    detail::_interpreter::get();

    PyObject* args = PyTuple_New(3);
    PyTuple_SetItem(args, 0, PyBool_FromLong((long int)enable));
    PyTuple_SetItem(args, 1, PyString_FromString(axis.c_str()));
    PyTuple_SetItem(args, 2, PyBool_FromLong((long int)tight));

    PyObject* res = PyObject_CallObject(detail::_interpreter::get().s_python_function_autoscale, args);
    
    Py_DECREF(args);
    
    if(!res) throw std::runtime_error("Call to autoscale() failed.");

    Py_DECREF(res);
}

inline void ticklabel_format(std::string axis="both", std::string style="sci", int scilimits_low=0, int scilimits_high=0, double useOffset=0., bool useLocale=false, bool useMathText=false)
{
    detail::_interpreter::get();

    PyObject *scilimits = PyTuple_New(2);
    PyTuple_SetItem(scilimits, 0, PyLong_FromLong((long int)scilimits_low));
    PyTuple_SetItem(scilimits, 1, PyLong_FromLong((long int)scilimits_high));

    PyObject* kwargs = PyDict_New();
    PyDict_SetItemString(kwargs, "axis", PyString_FromString(axis.c_str()));
    PyDict_SetItemString(kwargs, "style", PyString_FromString(style.c_str()));
    PyDict_SetItemString(kwargs, "scilimits", scilimits);
    PyDict_SetItemString(kwargs, "useOffset", PyFloat_FromDouble(useOffset));
    PyDict_SetItemString(kwargs, "useLocale", PyBool_FromLong((long int)useLocale));
    PyDict_SetItemString(kwargs, "useMathText", PyBool_FromLong((long int)useMathText));

    PyObject* args = PyTuple_New(0);

    PyObject* res = PyObject_Call(detail::_interpreter::get().s_python_function_ticklabel_format, args, kwargs);
    
    Py_DECREF(scilimits);
    Py_DECREF(args);
    Py_DECREF(kwargs);
    
    if(!res) throw std::runtime_error("Call to ticklabel_format() failed.");
...
inline void figure_size(size_t w, size_t h, size_t dpi=100)
{
    detail::_interpreter::get();
...
inline void tick_params(const std::map<std::string, std::string>& keywords, const std::string axis = "both")
{
  ...
  {
    if (it->second == "on")
        PyDict_SetItemString(kwargs, it->first.c_str(), PyBool_FromLong(1));
    else if (it->second == "" or it->second == "off")
        PyDict_SetItemString(kwargs, it->first.c_str(), PyBool_FromLong(0));
    else
        PyDict_SetItemString(kwargs, it->first.c_str(), PyString_FromString(it->second.c_str()));
  }
...
inline void tight_layout(double left=0, double bottom=0, double right=1, double top=1) {
    detail::_interpreter::get();

    PyObject* rect = PyTuple_New(4);
    PyTuple_SetItem(rect, 0, PyFloat_FromDouble(left));
    PyTuple_SetItem(rect, 1, PyFloat_FromDouble(bottom));
    PyTuple_SetItem(rect, 2, PyFloat_FromDouble(right));
    PyTuple_SetItem(rect, 3, PyFloat_FromDouble(top));

    PyObject* kwargs = PyDict_New();
    PyDict_SetItemString(kwargs, "rect", rect);
